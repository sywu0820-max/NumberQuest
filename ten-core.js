...
let state=createTenCoreState(),idleTimer=null,suppressTap=false;
...
function buildSurface(){
  ...
  for(const [index,id] of TEN_CORE_BALL_IDS.entries()){const ball=document.createElement('button');ball.type='button';ball.className='ball';ball.draggable=false;ball.dataset.ballId=id;ball.setAttribute('aria-label',`球 ${index+1}`);$('looseTray').append(ball);balls.set(id,ball);bindBall(ball)}
