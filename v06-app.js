import {
  ...
  makeDivisionQuestion,makeStoryQuestion,visualHintModel,takeDueReview,queueSpacedReview,completeSpacedReview,
  recordSkillMiss,recordSkillSuccess,skillMastery,challengeWeights,mixedSkillKeys,divisionUnlocked,
  beginLearningSession,finishRun,finishSpecialRun,claimReadyDaily
...
let runMode='world',challengeLevel=2,comboRewards=new Set(),introContinuation=null,activeUtterance=null;
...
function renderHome(){
  cancelSpeech();S=normalizeState(S,localDayKey());claimReadyDaily(S);save();renderDaily();renderLengths();renderBridge();renderWorlds();$('home').style.display='block';$('game').style.display='none'
}
function levelForLength(n){return n>=20?3:n>=10?2:1}
function resetRun(){correct=0;combo=0;maxCombo=0;locked=false;attempted=false;missRecorded=false;missCount=0;comboRewards=new Set();cancelSpeech();clearVisualHint();beginLearningSession(S);save()}
...
function next(){
  cancelSpeech();clearVisualHint();attempted=false;missRecorded=false;missCount=0;locked=false;q=takeDueReview(S);
  if(!q){if(runMode==='academy')q=makeMixedQuestion(S,{challengeLevel});else if(runMode==='focus')q=makeFocusQuestion(S,{challengeLevel});else if(runMode==='division')q=makeDivisionQuestion(S);else if(runMode==='story')q=makeStoryQuestion(S,{challengeLevel});else q=makeQuestion(wi,S,{challengeLevel})}
  save();
...
function renderQuestion(){
  ...
  $('msg').textContent=q.isReview?'🧠 這個能力回來了！試試看還記得嗎？':boss?'攻擊 Boss！':q.story?'先找出故事裡的數量關係！':'選一個答案！';$('answers').innerHTML='';
...
function answer(n,b){
  if(locked)return;
  if(n===q.ans){
    locked=true;cancelSpeech();correct++;S.daily.solved++;S.gems+=attempted?1:2;S.xp+=attempted?6:10;if(q.isReview&&!attempted)completeSpacedReview(S,q);recordSkillSuccess(S,q.skillKey,{firstTry:!attempted,isRevisit:Boolean(q.isReview)});
    ...
    attempted=true;missCount+=1;combo=0;if(!missRecorded){missRecorded=true;recordSkillMiss(S,q.skillKey);queueSpacedReview(S,q);save()}b.disabled=true;b.classList.add('wrong-choice');renderVisualHint(missCount>=2?2:1);$('msg').textContent=missCount>=2?`🧩 換個方式整理：${q.hint||'慢慢看圖，再試一次。'}`:(q.hint||'💡 差一點！看看小線索，再試一次。');updateBars()
  }
}
...
function finish(){
  ...
  $('resultEmoji').textContent=boss?'🎆':runMode==='academy'?'🏅':runMode==='focus'?'🎯':runMode==='division'?'🌉':runMode==='story'?'📖':'🏆';$('resultTitle').textContent=boss?'Boss 擊破！':runMode==='academy'?'20 題大師遠征完成！':runMode==='focus'?'弱點特訓完成！':runMode==='division'?'除法橋探險完成！':runMode==='story'?'故事任務完成！':`${totalNeeded} 題遠征完成！`;
...
$('homeBtn').onclick=renderHome;$('collectionBtn').onclick=showCollection;$('masteryBtn').onclick=showMastery;$('storyBtn').onclick=startStory;$('focusBtn').onclick=startFocus;$('academyBtn').onclick=startAcademy;$('divisionBtn').onclick=startDivision;$('readBtn').onclick=readQuestion;
