...
const QA_MODE=new URLSearchParams(location.search).get('qa')==='v09';
...
function hud(){
  ...
  const keys=[...MULTIPLICATION_SKILLS,...DIVISION_SKILLS,'add:20','sub:20','add:50','sub:50','add:100','sub:100',...NUMBER_SENSE_SKILLS];$('powerCount').textContent=keys.filter(key=>skillMastery(S,key).mastered).length;
}
...
function renderMemory(){const status=memoryChestStatus(S,{day:localDayKey()}),button=$('memoryBtn');button.disabled=!status.ready;button.classList.toggle('ready',status.ready);$('memoryStatus').textContent=status.label;$('journeyMemory').textContent=status.ready?`有 ${Math.min(2,status.dueCount)} 個記憶力量會加入今天的冒險`:'今天會找回、轉換，也會試一個小挑戰'}
...
function nextJourneyQuestion(){
  return takeNextJourneyQuestion(S,journeyQueue);
}
function next(){
  cancelSpeech();clearVisualHint();clearHelpChoices();attempted=false;missRecorded=false;missCount=0;locked=false;
  q=runMode==='memory'?memoryQueue[correct]:runMode==='journey'?nextJourneyQuestion():takeDueReview(S);
...
function answer(n,b){
  if(locked)return;
  if(n===q.ans){
    locked=true;cancelSpeech();correct++;S.daily.solved++;S.gems+=attempted?1:2;S.xp+=attempted?6:10;
    ...
    if(runMode==='journey'){const event={completed:true,purpose:q.journeyPurpose||'repair',skillKey:q.skillKey,representation:q.story?'story':'symbolic',isMemoryReview:Boolean(q.isMemoryReview),firstTry:!attempted,recovered:attempted,fingerprint:questionFingerprint(q)};journeyEvents.push(event);rememberJourneyEvent(S,event)}
    if(attempted){combo=0;$('msg').textContent=q.isMemoryReview?'✨ 找回來了！明天它會再發光。':q.isReview?'✨ 找到線索了！它之後還會再來。':'✨ 找到了！先繼續冒險，等等再挑戰一次。'}else{combo++;maxCombo=Math.max(maxCombo,combo);S.daily.maxCombo=Math.max(S.daily.maxCombo,maxCombo);if(combo>=10&&combo%10===0&&!comboRewards.has(combo)){comboRewards.add(combo);S.gems+=5;$('msg').textContent=`🌟 ${combo} 連擊寶箱！+💎5`}else $('msg').textContent=q.isMemoryReview?'✨ 這個能力記得更久了！':q.isReview?'🧠 自己想起來了！這個能力更亮了！':combo>=3?`⚡ ${combo} 連擊！超強！`:'🎉 一次答對！'}
...
function finish(){
  cancelSpeech();const out=runMode==='world'?finishRun(S,wi,{boss,maxCombo,questionCount:totalNeeded}):finishSpecialRun(S,{mode:runMode,maxCombo,questionCount:totalNeeded});save();const recap=runMode==='journey'?buildJourneyRecap(journeyEvents,{skillLabel}):null;
  $('resultEmoji').textContent=boss?'🎆':runMode==='journey'?'🧭':runMode==='memory'?'🧠':runMode==='academy'?'🏅':runMode==='focus'?'🎯':runMode==='division'?'🌉':runMode==='story'?'📖':'🏆';$('resultTitle').textContent=boss?'Boss 擊破！':runMode==='journey'?'今天的冒險完成！':runMode==='memory'?'記憶寶箱點亮了！':runMode==='academy'?'20 題大師遠征完成！':runMode==='focus'?'弱點特訓完成！':runMode==='division'?'除法橋探險完成！':runMode==='story'?'故事任務完成！':`${totalNeeded} 題遠征完成！`;
  $('resultText').textContent=runMode==='journey'?'你讓好幾個數字能力又亮了一點。':`今天又完成 ${totalNeeded} 個挑戰！${out.dailyBonus?` · 今日任務 +💎${out.dailyBonus}`:''}`;const recapRoot=$('capabilityRecap');recapRoot.innerHTML='';recapRoot.classList.toggle('visible',Boolean(recap));if(recap){for(const line of recap.lines)recapRoot.append(el('div','recap-line',line));if(!recap.lines.length)recapRoot.append(el('div','recap-line','🌱 今天完成了一段自己的數字冒險'))}
  $('newCollectible').textContent=out.collectible.isNew?`${out.collectible.icon} 新寶物：${out.collectible.name}${out.collectible.rare?' ✨稀有！':''}`:`${out.collectible.icon} ${out.collectible.name} 已收集，變成額外寶石！`;$('resultOverlay').style.display='flex';updateQaOutput();
}
...
function showMastery(){$('mulMastery').innerHTML='';MULTIPLICATION_SKILLS.forEach((key,i)=>$('mulMastery').append(badge(`${i+1}×`,key)));$('divMastery').innerHTML='';DIVISION_SKILLS.forEach((key,i)=>$('divMastery').append(badge(`${i+1}÷`,key)));$('divisionMasterySection').hidden=!divisionUnlocked(S);$('arithMastery').innerHTML=[masteryCard('20 內加法','add:20'),masteryCard('20 內減法','sub:20'),masteryCard('50 內加法','add:50'),masteryCard('50 內減法','sub:50'),masteryCard('100 內加法','add:100'),masteryCard('100 內減法','sub:100'),...NUMBER_SENSE_SKILLS.map(key=>masteryCard(skillLabel(key),key))].join('');const keys=mixedSkillKeys(S,{challengeLevel:3}),weights=challengeWeights(S,keys,{recentSkills:[]}),nextKey=[...keys].sort((a,b)=>weights[b]-weights[a])[0];$('nextAdventure').textContent=`🧭 下一段冒險：${skillLabel(nextKey)}。多遇見幾次，能力就會慢慢亮起來！`;$('masteryOverlay').style.display='flex'}

$('homeBtn').onclick=renderHome;$('journeyBtn').onclick=startJourney;$('collectionBtn').onclick=showCollection;$('masteryBtn').onclick=showMastery;$('memoryBtn').onclick=startMemory;$('storyBtn').onclick=startStory;$('focusBtn').onclick=startFocus;$('academyBtn').onclick=startAcademy;$('divisionBtn').onclick=startDivision;$('readBtn').onclick=readQuestion;
...
Object.defineProperty(window,'__NQ_V09_DEBUG__',{value:{getJourneyPlan:()=>journeyPlanSnapshot.map(item=>JSON.parse(JSON.stringify(item))),getJourneySummary:()=>journeyPlanSummary(journeyPlanSnapshot),getJourneyEvents:()=>journeyEvents.map(item=>({...item})),getStoryRun:()=>storyRunLog.map(item=>({...item})),getStoryDiversity:()=>storyDiversitySummary(storyRunLog.map(item=>({story:true,storyTemplateId:item.templateId,storyThemeId:item.themeId,txt:item.text}))),getCurrentQuestion:()=>q?JSON.parse(JSON.stringify(q)):null},configurable:false,writable:false});
