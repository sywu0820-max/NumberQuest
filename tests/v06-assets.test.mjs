...
test('current successor keeps the v0.6 dependency chain while isolating its active cache',()=>{
  assert.match(html,/v08-app\.js/);assert.match(html,/v06\.css/);assert.match(html,/v07\.css/);assert.doesNotMatch(html,/src="v06-app\.js"/);
  assert.match(worker,/number-quest-v08-/);assert.match(worker,/k\.startsWith\('number-quest-v08-'\)/);assert.doesNotMatch(worker,/caches\.match\(e\.request\)/);
  assert.match(worker,/v08-app\.js/);assert.match(worker,/src\/v08-core\.mjs/);assert.match(worker,/src\/v07-core\.mjs/);assert.match(worker,/src\/v06-core\.mjs/);assert.match(worker,/src\/v05-core\.mjs/);
});
