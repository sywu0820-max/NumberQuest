...
test('a review due before journey start is owned by the plan and asked once',()=>{
  ...
  const queue=planTodaysAdventure(s,{day:'2026-08-30',rng:seeded(6)}),seen=new Set(),asked=drainJourney(s,queue,seen);
...
test('Memory identity wins when the same fingerprint is also a due pending review',()=>{
  ...
  seen.add(fingerprint);completeMemoryRetrieval(s,first,{day:'2026-08-30',firstTry:false});s.learning.solvedTotal+=1;
