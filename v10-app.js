...
import {
  ...
  lanternHint,freshLanternRetry,lanternRunCompletion,lanternMasterySnapshot,lanternEligibility
...
function lanternQaOutput(){if(!QA_MODE)return;const output=$('lanternDebug');if(output)output.textContent=JSON.stringify({currentQuestion:lanternQuestion?JSON.parse(JSON.stringify(lanternQuestion)):null,plan:lanternPlanSnapshot,events:lanternEvents,completion:lanternRunCompletion(lanternEvents),mastery:lanternMasterySnapshot(S),eligibility:lanternEligibility({numberRange:LANTERN_NUMBER_RANGE}),pageErrors:[...(window.__NQ_PAGE_ERRORS__||[])]})}
...
function renderLanternControls(model){
  const root=$('lanternControls');root.innerHTML='';lanternSelection={value:Math.max(101,Math.min(LANTERN_NUMBER_RANGE,Number(lanternQuestion.result)-1)),hundreds:0,tens:0,ones:0};
  if(model.kind==='count'){
...
function startLantern(){
  resetRun();runMode='lantern';lanternQueue=planLanternRun(S,{count:8,day:localDayKey(),numberRange:LANTERN_NUMBER_RANGE});lanternPlanSnapshot=lanternQueue.map(item=>JSON.parse(JSON.stringify(item)));lanternEvents=[];lanternIndex=0;lanternQuestion=null;$('home').style.display='none';$('game').style.display='none';$('lanternWorld').style.display='block';renderLanternMission();
}
